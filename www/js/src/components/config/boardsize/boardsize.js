...
class Boardsize extends HTMLElement {
	...
	*/
	...
	connectedCallback() {
		...
		if (nb) {
			this.dispatchEvent(makeCustom('config/boardsize/ready', { self : this, nb : nb.textContent }))
		}
	}
	...
	_attachEvents() {
		...
		if (input && nb) {
			...
			input.onchange = (evt) => {
				const target = evt.currentTarget
				this.dispatchEvent(makeCustom('config/boardsize/size', { self : this, nb : target.value }))
			}
		}
	}
